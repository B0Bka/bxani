...
class CustomFilterSEFController
{
    ...
    public static function determineSefUrl($url = '', $urlOrigin = null)
    {
        ...
        elseif(in_array($urlOriginParts[1], ['catalog']))
        {   
            $urlFilterPart  = end($urlOriginParts);
            if($urlFilterPart == 'index.php'){
                $urlFilterPart = array_slice(
                    $urlOriginParts, count($urlOriginParts) - 2, 1
                );
            }
        }
        else
    ...
    public static function removeSefUrlPart($url)
    {
        ...
        $sectionsUrlParent = app('CatalogService')->getCatalogSectionsUrlParent();
        foreach($sectionsUrlParent as $sectionUrl){
            if(strpos($url, $sectionUrl) !== false && $sectionUrl !== "/"){
                if(substr($url, -9) === 'index.php'){
                    $sectionUrl.='index.php';
                    return $sectionUrl;
