...
class IblockElementModel extends AbstractModel implements SeoParamsInterface
{
    ...
    public function obtainSections($getProps = false)
    {
        ...
        if($getProps)
        {
            $arFilter = Array('IBLOCK_ID'=>PRODUCTS_IBLOCK_ID, 'ID'=>$arId);
            $db_list = \CIBlockSection::GetList(Array('ID'=>'ASC'), $arFilter, false, array('UF_*'));
            while($ar_result = $db_list->GetNext())
            {
              $this->sections[$ar_result['ID']] = $this->createSection($ar_result);
            }
        }
        return $this;
    }
     ...
     */
    public function getSections($getProps = false)
    {
        if(is_null($this->sections))
        {
            $this->obtainSections($getProps);
        }
     ...
     * @return mixed
     */
    public function getPropertyValue($propName, $index = false, $tilde = false)
    {
        return $this->getPropertyParam($propName, $index, 'VALUE', $tilde);
    }
    ...
    public function getPropertyValueXmlId($propName, $index = false)
    {
        return $this->getPropertyParam($propName, $index, "VALUE_XML_ID");
    }

    public function getPropertyValueHtml($propName, $index = false, $tilde = false)
    {
        return $this->getPropertyParam($propName, $index, '~VALUE', $tilde);
    }

    protected function getPropertyParam($propName, $index, $paramName = "VALUE", $tilde = false)
    {
        $result = false;
        if(!empty($propName))
        {
            $paramNameField = "PROPERTY_{$propName}_{$paramName}";
            if($tilde)
            {
                $paramNameField = "~{$paramNameField}";
            }
            if(array_key_exists($paramNameField, $this->fields))
            {
                $propValue = $this->fields[$paramNameField];
            }
            else
            {
                $paramName = ($tilde ? "~{$paramName}" : $paramName);
                $this->obtainProps();
    ...
    {
        return $this->fields['CREATED_USER_NAME'];
    }
}
