...
class Service
{
    ...
    public function getCatalogSectionsUrlParent()
    {
        static $sectionsUrl;
        if(!isset($sectionsUrl)){
            $sections = $this->sectionsRepository->getList(array('SORT' => 'ASC'));
    ...
    /**
     * Возвращает все разделы каталога, у которых стоит галка Принудительно показывать в распродаже
     * @return array
     */
    public function getCatalogSectionsSale()
    {
        static $sectionsSale;
        if(!isset($sectionsSale)){
            $sections = $this->sectionsRepository->getList(['SORT' => 'ASC'],['!UF_SALE'=>false]);
            foreach($sections as $section){
                $sectionsSale[] = $section->getId();
            }
        }
        return $sectionsSale;
    }
