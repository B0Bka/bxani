...
?>
<div class="one-cat-item gtm-parent" id="<?=$product->getID()?>" <?=Aniart\Main\Tools\Gtm::getDataStr($product)?>>
    ...
    <div class="one-cat-thumb" data-av="<?=$available?>">
        ...
        <div class="product_img loader">
            <a href="<?=$product->getDetailPageUrl()?>" class="gtm-link" data-id="<?=$product->getId();?>">
                <?if(!empty($imgData)):?>
                ...
                <?foreach($imgData as $key => $img):?>
                    <img
                            ...
                            width="<?=$img['width']?>"
                            height="<?=$img['height']?>"
                            alt="<?=$product->getName()?> - каталог Natali Bolgar"
                            title="<?=$product->getName()?> - фото <?$key+1?>"
	                    <?if($key == 0):?>
    ...
    <div class="one-cat-color">
        <?foreach($sibling['ITEMS'] as $key => $cloth):
            $firstPic = $key == 0 ? $imgData[0]['src'] : $cloth['photo'][0];
            $secondPic = $key == 0 ? $imgData[1]['src'] : $cloth['photo'][1];
            ?>
