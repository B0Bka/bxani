;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					this.setReloadOrder();//после изменения в корзине при переходе на второй шаг перегрузить компонент оформления
				}, this),
		...
		changeQuantitySelect: function(quantity, el)
		{
			var itemData = this.getItemDataByTarget(el);
			if (itemData)
			{
				this.setQuantity(itemData, quantity);
			}
		...
		getItemDataByValue: function(target)
		{
			var data = false;
			var id;
			var itemNode = BX.findParent(target, {attrs: {'data-entity': 'basket-item'}});
		...
		changeSku: function(val, target)
		{
			...
			var itemData = this.getItemDataByValue(target);
			if (itemData)
			{
		...
		changeSkuSelect: function(val, target)
		{
			var itemData = this.getItemDataByValue(target);
			if (itemData)
			{
		...
		clearBasket: function()
		{
			...
			BX.ajax({
				...
				onfailure: BX.delegate(function() {
					alert('fail');this.actionPool.doProcessing(false);
				}, this)
			});
		},
		setReloadOrder: function(){
			$('#order-ajax-block').addClass('reload');
		},
	};
})();