...
foreach ($this->basketItems as $row)
{
    ...
    if($rowData['FULL_PRICE'] > $rowData['PRICE']) $rowData['HAS_DISCOUNT'] = true;
	for($i = 1; $i <= $rowData['AVAILABLE_QUANTITY']; $i++)
    {
        $q = [
            'VALUE' => $i,
            'SELECTED' => $i == $rowData['QUANTITY'] ? true : false
        ];
        $rowData['QUANTITY_LIST'][] = $q;

    }
	$result['BASKET_ITEM_RENDER_DATA'][$row['PRODUCT_ID']] = $rowData;
}
...
foreach($basket as $item)
{
    ...
    $result['BASKET_ITEM_RENDER_DATA'][$id]['COLOR'] = $product->getClothImg();
    $result['BASKET_ITEM_RENDER_DATA'][$id]['NAME'] = $product->getName();
    $result['BASKET_ITEM_RENDER_DATA'][$id]['ARTICLE'] = $product->getArticle();
    foreach($product->getAllImagesId(1) as $key=>$img)
    {
        $img = $product->getMinPicture($img, 180, 270);
        $result['BASKET_ITEM_RENDER_DATA'][$id]['IMAGE_URL'] = $img['src'];
    }
}
$totalData = array(
...
if($result['allSum'] > FREE_DELIVERY_SUM)
{
    $totalData['DELIVERY_PRICE'] = i18n('FREE_DELIVERY');
}
else
{
    $allSum = $result['allSum'] + DELIVERY_SUM;
    $totalData['PRICE_FORMATED'] = SaleFormatCurrency($allSum, $result['CURRENCY']);
    $totalData['DELIVERY_PRICE'] = SaleFormatCurrency(DELIVERY_SUM, $result['CURRENCY']);
}
if(count($totalData['COUPON_LIST']) > 0)
    $totalData['SHOW_COUPON'] = 'block';
else
    $totalData['SHOW_COUPON'] = 'none';
