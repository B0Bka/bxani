...
)
{
	...
	if($arParams["SHOW_OTHERS"] === "Y")
	{
		...
		$obTitle = new CSearch;
        $str_query = $arResult["alt_query"]? $arResult["alt_query"]: $arResult["query"];
		if($obTitle->Search(array("QUERY"=>$str_query), array(), $exFILTER))
		{
			while($ar = $obTitle->Fetch())
			{
				$j++;
...
if($arElementsIds){
$arFilter = array('=ID' => $arElementsIds);

$arProduct = $productsRepository->getList(array(),$arFilter);
$arResult['CATEGORIES'][0]['PRODUCTS'] = $arProduct;
}
